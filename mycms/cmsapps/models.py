...
class CMSAppRegistry(models.Model):
    ...
    def get_api_urls(cls):
        ...
        try:
            ...
            #this can occur when the databse has not yet been populated.
            #The first python manage.py migrate will fix this.
            print("Warning: Database not ready , Could not load CMSAppRegistry because it does not exist yet.")
        return router.urls
    @classmethod
...
class CMSNode(models.Model):
    ...
    path = models.CharField(max_length=2000, db_index=True,unique=True,help_text="Path to the node. example: /cms/category/file")
    ...
    def get_pageview(self,request):
        ...
        PageView = self.cmsapp.pageview_class
        pageview = PageView(self) #Pass ourself into the PageView so it knows what node it is.
    ...
    def save(self, *args, **kwargs):
        """
        Override the models save method to check if a slug was given and
        if not, then use the title to create a slug.
        """
...
class CMSTextField(models.Model):
    """
    This has a default textarea
    """
...
class CMSCharField(models.Model):
